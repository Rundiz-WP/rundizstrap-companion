...
if (!trait_exists('\\BBFSEPlugin\\App\\AppTrait')) {
    trait AppTrait
    {
         ...
         * Main option name.
         * 
         * @var string Set main option name of this plugin. the name should be english, number, underscore, or anycharacters that can be set to variable. for example: 'bbfse_plugin_optname' will be set to $bbfse_plugin_optname
         * @uses call this trait method $this->getOptions(); before access $bbfse_plugin_optname in global variable.
         */
        public $main_option_name = 'bbfse_plugin_optname';

        /**
         * All available options.
         * 
         * These options will be accessible via main option name variable. for example: options name 'the_name' can call from $bbfse_plugin_optname['the_name'];.
         ...
         * If you have tables to create on activate this plugin then set the db version number (string) here and then write create table schema at \BBFSEPlugin\App\Models\PluginDbStructure->get() method.
        ...
        public function setupAllOptions()
        {
            ...
            if (is_array($config_values) && array_key_exists('rundiz_settings_config_file', $config_values)) {
                ...
                wp_die(
                    esc_html(__('Settings configuration file was not set.', 'bbfse-plugin'))
                );
                exit;
            }
