...
if (!class_exists('\\RundizstrapCompanion\\App\\Libraries\\RundizSettings')) {
    ...
    class RundizSettings
    {
         ...
         * 
         * @since 2026-07-28
         * @param \RundizstrapCompanion\App\Libraries\Loader $Loader The `Loader` class.
                        ...
                        if (array_key_exists('type', $fields) && 'checkbox' === $fields['type'] && array_key_exists('options', $fields) && is_array($fields['options'])) {
                            ...
                            foreach ($fields['options'] as $checkbox_key => $checkboxes) {
                                ...
                                if (array_key_exists('id', $checkboxes)) {
                                    ...
                                    $field->value = ($checkboxes['value'] ?? null);
                                    $field->input_attributes = ($checkboxes['input_attributes'] ?? null);
                            ...
                            if (array_key_exists('id', $fields)) {
                                $field = new \stdClass();
                                $field->type = ($fields['type'] ?? null);
                                $field->default = $default;
                                $field->value = ($fields['value'] ?? null);
                                $field->input_attributes = ($fields['input_attributes'] ?? null);
                                $field->select_options = $select_options;
                                $field->sanitize_callback = ($fields['sanitize_callback'] ?? null);
                                $field->save_callback = ($fields['save_callback'] ?? null);
                                $field->display_callback = ($fields['display_callback'] ?? null);
         ...
         */
        public function getSettingsFieldsId(): array
        {
         ...
         */
        public function processDisplayCallback(array $data): array
        {
            $fields = $this->getSettingsFields();
         ...
         */
        public function processSaveCallback(array $data): array
        {
            $fields = $this->getSettingsFields();
         ...
         */
        private function renderFormCodeEditor(int $field_key, array $fields, array $options_values = []): string
        {
         ...
         * @return string Return rendered input.
         */
        private function renderFormEditor(int $field_key, array $fields, array $options_values = []): string
        {
            $field_name = (array_key_exists('id', $fields) ? $fields['id'] : $field_key);
            if (is_array($options_values) && array_key_exists($field_name, $options_values)) {
                $field_value = $options_values[$field_name];
         ...
         * @return string Return rendered input.
         */
        private function renderFormInput(int $field_key, array $fields, array $options_values = []): string
        {
         ...
         * @return string Return rendered input.
         */
        private function renderFormInputCheckbox(int $field_key, array $fields, array $options_values = []): string
        {
         ...
         * @return string Return rendered input.
         */
        private function renderFormInputRadio(int $field_key, array $fields, array $options_values = []): string
        {
         ...
         * @return string Return rendered input.
         */
        private function renderFormSelectbox(int $field_key, array $fields, array $options_values = []): string
        {
            $field_type = (array_key_exists('type', $fields) ? $fields['type'] : 'text');
            $field_name = (array_key_exists('id', $fields) ? $fields['id'] : $field_key);
            // check values
         ...
         * @return string Return rendered input.
         */
        private function renderFormTextarea(int $field_key, array $fields, array $options_values = []): string
        {
            $field_type = (array_key_exists('type', $fields) ? $fields['type'] : 'text');
            $field_name = (array_key_exists('id', $fields) ? $fields['id'] : $field_key);
            // check values
