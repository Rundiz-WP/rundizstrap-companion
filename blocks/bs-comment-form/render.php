...
$fields = [
    'author' => '<div class="mb-3 comment-form-author">'
        . '<label class="form-label" for="author">' . esc_html__('Name', 'bbfse-plugin') . ($requireNameEmail ? ' <span class="required">*</span>' : '') . '</label>'
    ...
    'email' => '<div class="mb-3 comment-form-email">'
        . '<label class="form-label" for="email">' . esc_html__('Email', 'bbfse-plugin') . ($requireNameEmail ? ' <span class="required">*</span>' : '') . '</label>'
    ...
    'url' => '<div class="mb-3 comment-form-url">'
        . '<label class="form-label" for="url">' . esc_html__('Website', 'bbfse-plugin') . '</label>'
...
if (has_action('set_comment_cookies', 'wp_set_comment_cookies') && get_option('show_comments_cookies_opt_in')) {
    ...
    $fields['cookies'] = '<div class="mb-3 comment-form-cookies-consent form-check">'
        ...
        . '<label class="form-check-label" for="wp-comment-cookies-consent">' . esc_html__('Save my name, email, and website in this browser for the next time I comment.', 'bbfse-plugin') . '</label>'
        . '</div>';
    unset($consent);
}
unset($commenter);
...
$commentFormArgs = [
    ...
    'comment_field' => '<div class="mb-3 comment-form-comment">'
        . '<label class="form-label" for="comment">' . esc_html__('Comment', 'bbfse-plugin') . '</label>'
